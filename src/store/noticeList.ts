...
export const useMyStore = create<Notice>((set)=>({
    ...
    Action: async () => {
        ...
        try {
            set({loading : true})
            const response = await axios.get(`https://apis.data.go.kr/1262000/NoticeService2/getNoticeList2?serviceKey=${REACT_API_KEY}&returnType=JSON&numOfRows=10&pageNo=1`);
